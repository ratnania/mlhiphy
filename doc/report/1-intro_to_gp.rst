...
Gaussian processes and the covariance function
--------------------------------------------------


**Definition** (Stochastic process) Given a set :math:`\mathrm{T} \subset \mathbb{R}`, a measurable space  :math:`( H , \mathcal{H} )`, and a probability space :math:`(\Omega, \mathcal{F}, \mathbb{P})`, a H-valued stochastic process is a set of H-valued random variables :math:`\{X(t): t \in \mathrm{T}\}`. We simply write X(t) to denote the process. To exphasise the dependence on :math:`\omega` and that :math:`X : \Omega \times \mathrm{T} \rightarrow \mathbb{R}`, we may write :math:`X(t,\omega)`.

**Definition** (second order) A real-valued stochastic process is second order if :math:`X(t) \in L^2(\Omega)` for each :math:`t \in \mathrm{T}`. The mean function is defined by :math:`\mu(t) := \mathbb{E}[X(t)]` and the covariance function is defined by :math:`C(s, t) : = Cov(X(s), X(t)))` for all :math:`s,t \in \mathrm{T}`.

**Theorem** Let :math:`\mathrm{T} \subset \mathbb{R}`. The following statements are equivalent.
...
(2) The function :math:`\mu` maps from :math:`\mathrm{T} \rightarrow \mathbb{R}` and the function :math:`C` maps from :math:`\mathrm{T} \times \mathrm{T} \rightarrow \mathbb{R}`. Further C is symmetric and non-negative definite.
...
and :math:`t_1, \dotsc, t_N \in \mathrm{T}`
...
:math:`C(s, t)` is symmetric as :math:`Cov(X(s), X(t)) = Cov(X(t), X(s))` for all :math:`t,s \in \mathrm{T}`.
